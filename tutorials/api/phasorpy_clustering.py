...
mean_filtered, real_filtered, imag_filtered = phasor_filter_median(mean, real, imag, size=3, repeat=2)

mean_filtered, real_filtered, imag_filtered = phasor_threshold(mean_filtered, real_filtered, imag_filtered, mean_min=1)
...
phase_center = []
for i in range(len(centers_real)):
    modulation_center.append( numpy.sqrt( centers_real[i]**2 + centers_imag[i]**2 ) )
...
modulation_min = [(modulation_center[0] - delta), (modulation_center[0] - delta)]
modulation_max = [(modulation_center[0] + delta), (modulation_center[0] + delta)]
