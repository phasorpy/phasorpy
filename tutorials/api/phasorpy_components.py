...
plot1 = PhasorPlot(allquadrants=True, 
                   title='Components: First Harmonic Phasor Plot')
plot2 = PhasorPlot(allquadrants=True, 
                   title='Components: Second Harmonic Phasor Plot')
...
for i, img in enumerate(images):
    ...
    plot1.plot(real_center_h1, imag_center_h1, marker='o', 
               markersize=8, label=components[i])

    # Plot second harmonic
    plot2.hist2d(real[1], imag[1], cmap='RdYlBu_r', bins=300)
    plot2.plot(real_center_h2, imag_center_h2, marker='o', 
               markersize=8, label=components[i])
...
test_image = signal_from_lsm(fetch(
...
# Perform unmixing
fractions = numpy.asarray(phasor_based_unmixing(
    real, imag, matrixA, use_scipy=True))
