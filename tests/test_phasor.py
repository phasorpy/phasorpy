...
def test_phasor_from_signal():
    ...
    dc, real, imag = phasor_from_signal(signal=SYNTH_DATA_PHASOR_FROM_SIGNAL, 
                                        fft=numpy.fft.fft)
    assert dc.shape == (SYNTH_DATA_PHASOR_FROM_SIGNAL.shape[1], 
                        SYNTH_DATA_PHASOR_FROM_SIGNAL.shape[2])
    assert real.shape == (SYNTH_DATA_PHASOR_FROM_SIGNAL.shape[1], 
                          SYNTH_DATA_PHASOR_FROM_SIGNAL.shape[2])
    assert imag.shape == (SYNTH_DATA_PHASOR_FROM_SIGNAL.shape[1], 
                          SYNTH_DATA_PHASOR_FROM_SIGNAL.shape[2])
...
def test_empty_phasor_from_signal():
    """Test empty image phasor calculation error"""
    empty_data = numpy.zeros((32, 64, 64))
    with pytest.raises(ValueError):
        phasor_from_signal(empty_data, numpy.fft.fft)
